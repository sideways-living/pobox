# pobox.watch Architecture

pobox.watch separates four concepts:

- User identity belongs to an individual person.
- PO box state belongs to the shared workspace.
...
Users join workspaces through `workspace_members`. A PO box belongs to one workspace and one post office. A user may later belong to multiple workspaces without duplicating PO box state.
...
Duplicate protection is message-level, not day-level. If new mail arrives after collection, the PO box returns to waiting.
...
Collection is explicit and idempotent. If a PO box has already been collected, later collection attempts receive a conflict instead of creating contradictory state.
