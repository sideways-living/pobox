...
try {
  ...
  assert.equal(await page.locator(".summary-band").isVisible(), false);
  ...
  assert.match(await john.getByText(/Active · Member/).innerText(), /Active · Member/);
  for (const title of ["Edit user", "Reset password", "Disable user", "Delete user"]) assert.equal(await john.getByTitle(title, { exact: true }).count(), 1);
  await john.getByTitle("Edit user", { exact: true }).click();
  const johnEdit = directory.locator("form.team-member");
  assert.equal(await johnEdit.getByLabel("Email", { exact: true }).inputValue(), "john@example.com");
  await johnEdit.getByLabel("Emoji", { exact: true }).fill("📬");
  await johnEdit.getByRole("button", { name: "Save", exact: true }).click();
  await john.locator(".user-avatar").getByText("📬", { exact: true }).waitFor();
  page.once("dialog", dialog => dialog.dismiss());
  await john.getByTitle("Delete user", { exact: true }).click();
  assert.equal(await john.count(), 1);
  page.once("dialog", dialog => dialog.accept());
  await john.getByTitle("Delete user", { exact: true }).click();
  await deleted.getByText("john@example.com", { exact: true }).waitFor();
  await deleted.locator(".user-avatar").getByText("📬", { exact: true }).waitFor();
  ...
  assert.ok(await page.locator(".sidebar-header img").evaluate(img => img.complete && img.naturalWidth > 0));
  await page.getByRole("button", { name: "Open menu", exact: true }).click();
  assert.equal(await page.getByRole("button", { name: "Close menu", exact: true }).getAttribute("aria-expanded"), "true");
  assert.equal(await page.locator(".mobile-account").isVisible(), true);
  assert.equal(await page.getByRole("button", { name: "Log Out", exact: true }).isVisible(), true);
  await page.screenshot({ path: "/tmp/pobox-mobile-menu-open.png", fullPage: true });
  ...
  const profile = page.locator("section.panel").filter({ has: page.getByRole("heading", { name: "Profile Image", exact: true }) });
  await profile.getByLabel("Emoji", { exact: true }).fill("👤");
  await profile.getByRole("button", { name: "Save Profile Image", exact: true }).click();
  await profile.locator(".user-avatar").getByText("👤", { exact: true }).waitFor();
  assert.equal(await page.locator("#header-menu").isVisible(), false);
  await page.getByRole("button", { name: "Open menu", exact: true }).click();
  await page.mouse.click(375, 820);
  assert.equal(await page.locator("#header-menu").isVisible(), false);
  await page.setViewportSize({ width: 1440, height: 1000 });
